# -*- coding: utf-8 -*-
"""
Created on Sat Jun  3 14:35:56 2023
...
for j in range(len(epoch)):
    ...
    plt.title('Epoch '+ str(num_epochs[j]), fontsize=20)
    ...
    plt.grid(b=True,axis='y',alpha=0.3)
    plt.title('Epoch '+ str(epoch[j]), fontsize=20)
    plt.xlabel('Max Similarity',fontsize=20)
    plt.ylabel('Count',fontsize=20)
    plt.show()
    ...
    plot.savefig(outdir + '/similarity_epoch' + str(num_epochs[j]) + '.pdf')
    np.sum(np.where(np.array(whole2) > 0.75, 1,0))  # 891- 60%
    
    #np.save(outdir + '/MaxSimilarity_' + ep  + '-batch' + str(batch_size[j]), whole2)
    np.save(outdir + '/MaxSimilarity_epoch' + str(num_epochs[j]) + '-batch' + str(batch_size[j]), whole2)
    
    count = 0
    #with open('D:/PhD thesis/GCN/My Code/' + method + '/' + ep + '/log_rmv_0.txt','w') as f:
    with open(data_file + 'results/' + method + '/' + ep + '/epoch[j]' + str(num_epochs[j]) + '-batch' + str(batch_size[j]) +'log_rmv_0.txt','w') as f:
        for item1 in seq:
            total = []
            for item2 in raw:
                total.append((item2,similar(item1,item2)))
