# -*- coding: utf-8 -*-
"""
Created on Mon May 22 23:26:02 2023
...
class label_Generator(nn.Module):
    def __init__(self,label_size,hidden,seq_len,n_chars,batch_size):
        super(label_Generator,self).__init__()
        self.fc1 = nn.Linear(128+label_size,hidden*seq_len)
        self.block = nn.Sequential(
            ResBlock(hidden),
            ResBlock(hidden),
            ResBlock(hidden),
...
class label_Discriminator(nn.Module):
    def __init__(self,label_size,hidden,n_chars,seq_len):
        ...
        self.fc = nn.Linear(seq_len*hidden+label_size,1)
        self.hidden = hidden
        self.n_chars = n_chars
        self.seq_len = seq_len

    def forward(self,input,label):  # input [N,seq_len,n_chars]
        ...
        output = torch.concatenate((output,torch.tile(label,(input.shape[0],1))) , axis=1)
...
# output_dir = data_file + 'data/WGAN-GP/epoch' + str(num_epochs)
output_dir = data_file + 'results/WGAN-GP/epoch' + str(num_epochs)
